...
function M.server_ready() return not not vim.lsp.buf_notify(0, "window/progress", {}) end
...
function M.format(options)
  ...
  if not range and mode == "v" or mode == "V" then range = range_from_selection(bufnr, mode) end
  local method = range and "textDocument/rangeFormatting" or "textDocument/formatting"

  clients = vim.tbl_filter(function(client) return client.supports_method(method) end, clients)

  if #clients == 0 then vim.notify "[LSP] Format request failed, no matching language servers." end
...
function M.rename(new_name, options)
  ...
  if #clients == 0 then vim.notify "[LSP] Rename, no matching language servers with rename capability." end
...
local function pick_call_hierarchy_item(call_hierarchy_items)
  ...
  if #call_hierarchy_items == 1 then return call_hierarchy_items[1] end
  local items = {}
  ...
  if choice < 1 or choice > #items then return end
  return choice
end
...
function M.incoming_calls() call_hierarchy "callHierarchy/incomingCalls" end

--- Lists all the items that are called by the symbol under the
--- cursor in the |quickfix| window. If the symbol can resolve to
--- multiple items, the user can pick one in the |inputlist()|.
function M.outgoing_calls() call_hierarchy "callHierarchy/outgoingCalls" end
...
function M.add_workspace_folder(workspace_folder)
  ...
  for _, client in pairs(vim.lsp.get_active_clients { bufnr = 0 }) do
    ...
    if not found then
      ...
      if not client.workspace_folders then client.workspace_folders = {} end
...
---
function M.clear_references() util.buf_clear_references() end
...
local function on_code_action_results(results, ctx, options)
  ...
  local function action_filter(a)
    ...
    if options and options.filter and not options.filter(a) then return false end
  ...
  local function apply_action(action, client)
    if action.edit then util.apply_workspace_edit(action.edit, client.offset_encoding) end
...
function M.code_action(options)
  ...
  if options.diagnostics or options.only then options = { options = options } end
  local context = options.context or {}
  if not context.triggerKind then context.triggerKind = vim.lsp.protocol.CodeActionTriggerKind.Invoked end
